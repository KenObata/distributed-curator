...
class ProcessPartitionLocallyTest extends AnyFunSuite {
    ...
    test("identical documents found as similar pair") {
       ...
       val resultDataset: Dataset[ProcessPartitionLocallyUDF.SimilarityPairs] = resultDf.as[ProcessPartitionLocallyUDF.SimilarityPairs]
    ...
    test("similarity threshold filters correctly") {
        ...
        val lowThresholdDataset: Dataset[ProcessPartitionLocallyUDF.SimilarityPairs] = lowThresholdDf.as[ProcessPartitionLocallyUDF.SimilarityPairs]
        val lowThresholdArray: Array[ProcessPartitionLocallyUDF.SimilarityPairs] =  lowThresholdDataset.collect() //to access each index
    ...
    test("estimateSimilarity correctness") {
        ...
        */
        val estimateSimilarityMethod:Method = ProcessPartitionLocallyUDF.getClass.getDeclaredMethod(
            "estimateSimilarity",
            classOf[Array[Long]], classOf[Array[Long]]
        )
        estimateSimilarityMethod.setAccessible(true)

        //                             (object or class name, args)
        val identicalResult:Double = estimateSimilarityMethod.invoke(ProcessPartitionLocallyUDF, Array(1L, 2L), Array(1L, 2L))
        .asInstanceOf[Double]

        assert(identicalResult == 1.0)
        
        val halfMatchResult:Double = estimateSimilarityMethod.invoke(ProcessPartitionLocallyUDF, Array(1L, 2L), Array(1L, 4L))
        .asInstanceOf[Double]

        assert(halfMatchResult == 0.5)

        val noMatchResult:Double = estimateSimilarityMethod.invoke(ProcessPartitionLocallyUDF, Array(1L, 2L), Array(3L, 4L))
        .asInstanceOf[Double]
