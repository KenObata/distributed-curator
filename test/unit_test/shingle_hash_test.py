...
def test_hash_correctness():
    ...
    # Compare
    assert len(cython_hashes) == len(python_hashes), \
        f"Length mismatch: {len(cython_hashes)} vs {len(python_hashes)}"
...
def test_performance():
    ...
    for _ in range(iterations):
        ...
        python_result = np.array(
            [hash(s) & 0xFFFFFFFF for s in unique_shingles], 
            dtype=np.uint32
        )
