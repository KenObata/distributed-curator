...
def test_performance_batch():
    ...
    cython_result = compute_minhash_cython_batch(texts, num_hashes, ngram)
    cython_time = time.perf_counter() - start
    
    start = time.perf_counter()
    python_result = compute_minhash_vectorized_batch_only_hash_once(texts, num_hashes, ngram)
    python_time = time.perf_counter() - start
    
    print(f"\nBatch performance (1000 docs × ~5KB):")
